...
class ComedyHomeModel with ChangeNotifier, ItemListModel<Feed> {
  //=
  ...
  void init() {
    _feedController.addPageRequestListener((pageKey) {
      _fetchComediansHomeFeed(pageKey);
    });
  }
  ...
  Future<void> _fetchComediansHomeFeed(int pageKey) async {
    try {
      // Cancel current operation (if any)
      _feedOp?.cancel();

      // Create Request
      final request = ComedyFeedRequest(page: pageKey);
      _feedOp = async.CancelableOperation.fromFuture(
        locator<KwotData>().dashboardRepository.fetchComedyFeed(request),
        onCancel: () {
