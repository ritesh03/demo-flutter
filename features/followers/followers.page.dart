...
class _FollowersPageState extends PageState<FollowersPage> {
  //=

  @override
  void initState() {
    super.initState();
    followersModelOf(context).init();
  }
  ...
  Widget build(BuildContext context) {
    return SafeArea(
        top: false,
        child: Scaffold(
            body: ItemListWidget<User, FollowersModel>(
  ...
  Widget _buildTitle() {
    return Selector<FollowersModel, String?>(
        selector: (_, model) => model.userName,
        builder: (_, userName, __) {
          final title = (userName != null && userName.isNotEmpty)
              ? LocaleResources.of(context).followersOfUserFormat(userName)
              : LocaleResources.of(context).followers;
  ...
  Widget _buildSearchBar() {
    return Padding(
        ...
        child: SearchBar(
          ...
          onQueryChanged: followersModelOf(context).updateSearchQuery,
          onQueryCleared: followersModelOf(context).clearSearchQuery,
        ));
  }

  FollowersModel followersModelOf(BuildContext context) {
    return context.read<FollowersModel>();
  }
